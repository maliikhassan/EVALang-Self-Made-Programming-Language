...
class Parser:
  ...
  def expr(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'whisker'):
      res.register_advancement()
      self.advance()
  ...
  def atom(self):
    ...
    elif tok.matches(TT_KEYWORD, 'if_meow'):
      if_expr = res.register(self.if_expr())
      if res.error: return res
      return res.success(if_expr)

    elif tok.matches(TT_KEYWORD, 'for_paw'):
      for_expr = res.register(self.for_expr())
      if res.error: return res
      return res.success(for_expr)

    elif tok.matches(TT_KEYWORD, 'while_tail_wag'):
  ...
  def if_expr(self):
    res = ParseResult()
    all_cases = res.register(self.if_expr_cases('if_meow'))
    if res.error: return res
    cases, else_case = all_cases
    return res.success(IfNode(cases, else_case))

  def if_expr_b(self):
    return self.if_expr_cases('else_pounce')
    
  def if_expr_c(self):
    res = ParseResult()
    else_case = None

    if self.current_tok.matches(TT_KEYWORD, 'else_nap'):
      res.register_advancement()
      self.advance()
      ...
      if self.current_tok.type == TT_NEWLINE:
        ...
        if self.current_tok.matches(TT_KEYWORD, 'end_nap'):
          res.register_advancement()
          self.advance()
        else:
          return res.failure(InvalidSyntaxError(
  ...
  def if_expr_b_or_c(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'else_pounce'):
  ...
  def for_expr(self):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'for_paw'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'for_paw'"
      ))
    ...
    if self.current_tok.type == TT_NEWLINE:
      ...
      if not self.current_tok.matches(TT_KEYWORD, 'end_nap'):
        return res.failure(InvalidSyntaxError(
          self.current_tok.pos_start, self.current_tok.pos_end,
          f"Expected 'end_nap'"
        ))
  ...
  def while_expr(self):
    ...
    if not self.current_tok.matches(TT_KEYWORD, 'while_tail_wag'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'while_tail_wag'"
      ))
